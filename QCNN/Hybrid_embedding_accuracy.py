...
def Benchmarking_Hybrid_Accuracy(
    dataset, classes, Unitary, U_num_param, Encodings, circuit, binary=True
):
    U = Unitary
    ...
    for j in range(J):
        ...
        for n in range(5):
            ...
            loss_history, trained_params = Training.circuit_training(
                X_train, Y_train, U, U_params, Embedding, circuit
            )
            predictions = [
                QCNN_circuit.QCNN(x, trained_params, U, U_params, Embedding) for x in X_test
            ]
...
def Benchmarking_Hybrid_Entanglement(
    dataset, classes, Encodings, N_samples, best_trained_params_list
):
    for i in range(len(Encodings)):
        ...
        X_train, X_test, Y_train, Y_test = data.data_load_and_process(
            dataset=dataset, classes=classes, feature_reduction=Encoding, binary=True
        )
